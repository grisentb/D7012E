...
findMin :: [(Int,Int)]->[Int]->(Int,Int) -- FindMin(pairs of tuples (i,j), array) -> the tuples which return smallest sum
findMin [] array = (0,0)
findMin (x:xs) array = minOfTwoArrays x (findMin xs array) array

--Returns the k smallest tuple pairs which returns the smallest sums
kSmallest :: [(Int,Int)]->[Int]->Int->[(Int,Int)] --kSmallest(all possibles subarrays index, array, k) -> List of tuples: (size, sublist, i, j)
kSmallest subArrs array k
    |k>0        = [findMin subArrs array] ++ kSmallest (removeItem (findMin subArrs array) subArrs) array (k-1)
    |otherwise  = []